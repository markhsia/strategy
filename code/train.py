...
def dropout(x, pdrop, train):
    if add_noise == 'd': #dropout
        return tf.nn.dropout(x, 1-pdrop)
    if add_noise == 'g1': #gaussian+
        return x + tf.random_normal(shape=tf.shape(x), mean=0.0, stddev = 0.05, dtype=tf.float32)
    if add_noise == 'g2': #gaussian*
        return x + tf.multiply(tf.random_normal(shape=tf.shape(x), mean=0.0, stddev = 0.05, dtype=tf.float32), x)        
...
def iter_predict(Xs, Ms):
    ...
    for xmb, mmb in iter_data(Xs, Ms, n_batch=n_batch_train, truncate=False, verbose=True):
        n = len(xmb)
        if n == n_batch_train:
            result = sess.run([eval_mgpu_logits, eval_mgpu_lm_losses], {X_train:xmb, M_train:mmb})
        else:
            result = sess.run([eval_logits, eval_lm_losses], {X:xmb, M:mmb})
        logits.append(result[0])
        lmlosses.append(result[1])
        
    logits = np.concatenate(logits, 0)
    lmlosses = np.concatenate(lmlosses, 0)
    return logits, lmlosses
...
def predict_dev():
    ...
    pred_logits, pred_lm = iter_predict(vaX, vaM)
    path = os.path.join(submission_dir, "pred_dev_logits.txt")
    os.makedirs(os.path.dirname(path), exist_ok=True)
    with open(path, "w") as f:
        for i in range(len(pred_logits)):
            for j in range(4):
                f.write(str(pred_logits[i][j]) + (' ' if j != 3 else '\n'))

def predict():
    filename = filenames[dataset]
    pred_fn = pred_fns[dataset]
    label_decoder = label_decoders[dataset]
    pred_logits, pred_lm = iter_predict(teX, teM)
    predictions = pred_fn(pred_logits)
    ...
    with open(path, 'w') as f:
        ...
        for i, prediction in enumerate(predictions):
            f.write('{}\t{}\n'.format(i, prediction))
    path = os.path.join(submission_dir, "pred_test_logits.txt")
    with open(path, "w") as f:
        for i in range(len(pred_logits)):
            for j in range(4):
                f.write(str(pred_logits[i][j]) + (' ' if j != 3 else '\n'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--desc', type=str, default='race')
    ...
    (trX1, trX2, trX3, trX4, trX5, trX6, trX7, trX8, trX9, trY), (vaX1, vaX2, vaX3, vaX4, vaX5, vaX6, vaX7, vaX8, vaX9, vaY), (teX1, teX2, teX3, teX4, teX5, teX6, teX7, teX8, teX9) = encode_dataset(race(data_dir), encoder=text_encoder)
    ...
    X_train = tf.placeholder(tf.int32, [n_batch_train, 4, n_ctx, 3])
    M_train = tf.placeholder(tf.float32, [n_batch_train, 4, n_ctx])
    X = tf.placeholder(tf.int32, [None, 4, n_ctx, 3])
    ...
    print(len(init_params[0][:n_ctx]))
    init_params[0] = np.concatenate([init_params[0][:n_ctx], np.random.randn(n_ctx - len(init_params[0][:n_ctx]), n_embd)*0.02], 0)
    ...
    for i in range(n_iter):
        for xmb, mmb, ymb in iter_data(*shuffle(trX, trM, trYt, random_state=np.random), n_batch=n_batch_train, truncate=True, verbose=True):
            ...
            n_updates += 1
            if n_updates in [1000, 4000] and n_epochs == 0:
                log()
        n_epochs += 1
        log()
    if submit:
